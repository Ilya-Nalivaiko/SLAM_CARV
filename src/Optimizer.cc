                                 ...
                                 int nIterations, bool* pbStopFlag, const unsigned long nLoopKF, const bool bRobust)
{
    ...
    for(size_t i=0; i<vpMP.size(); i++)
    {
        ...
        for(map<KeyFrame*,size_t>::const_iterator mit=observations.begin(); mit!=observations.end(); mit++)
        {
            ...
            if(!pKF || pKF->isBad() || activeKFIds.find(pKF->mnId)==activeKFIds.end())
                continue;
            ...
            // SAFETY: octave bounds
            if(kpUn.octave < 0 || static_cast<size_t>(kpUn.octave) >= pKF->mvInvLevelSigma2.size())
                continue;

            // SAFETY: finite check
            if(!std::isfinite(kpUn.pt.x) || !std::isfinite(kpUn.pt.y))
                continue;

            nEdges++;

            // Monocular vs stereo
            const bool mono = (idx >= pKF->mvuRight.size()) || (pKF->mvuRight[idx] < 0);
            ...
            {
                const float kp_ur = pKF->mvuRight[idx];
                // SAFETY: finite
                if(!std::isfinite(kp_ur)) continue;
    ...
    for(size_t i=0; i<vpKFs.size(); i++)
    {
        ...
        if(!pKF || pKF->isBad())
            continue;
        // SAFETY: vertex may have been skipped
...
{    
    ...
    // SAFETY: track which KF vertices are actually in the optimizer
    std::unordered_set<unsigned long> activeKFIds;
    ...
    for(list<KeyFrame*>::iterator lit=lLocalKeyFrames.begin(), lend=lLocalKeyFrames.end(); lit!=lend; lit++)
    {
        ...
        optimizer.addVertex(vSE3);
        activeKFIds.insert(pKFi->mnId); // SAFETY
    ...
    for(list<MapPoint*>::iterator lit=lLocalMapPoints.begin(), lend=lLocalMapPoints.end(); lit!=lend; lit++)
    {
        ...
        for(map<KeyFrame*,size_t>::const_iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
        {
            ...
            if(!pKFi || pKFi->isBad()) continue;
            // SAFETY: only wire edges to KF vertices we actually inserted
            if(activeKFIds.find(pKFi->mnId)==activeKFIds.end()) continue;

            // SAFETY: bounds
            const size_t idx = mit->second;
            if(idx >= pKFi->mvKeysUn.size()) continue;

            const cv::KeyPoint &kpUn = pKFi->mvKeysUn[idx];
            if(kpUn.octave < 0 || static_cast<size_t>(kpUn.octave) >= pKFi->mvInvLevelSigma2.size()) continue;
            if(!std::isfinite(kpUn.pt.x) || !std::isfinite(kpUn.pt.y)) continue;

            // Monocular observation
            if(idx >= pKFi->mvuRight.size() || pKFi->mvuRight[idx]<0)
            {
                Eigen::Matrix<double,2,1> obs;
                obs << kpUn.pt.x, kpUn.pt.y;
    ...
    if(pbStopFlag)
        if(__atomic_load_n(pbStopFlag, __ATOMIC_RELAXED))
            return;
    ...
    // Get Map Mutex
    unique_lock<shared_mutex> lock(pMap->mMutexMapUpdate);
